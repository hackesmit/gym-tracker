...
function prIcon(type) {
  ...
  if (type === 'weight_pr') return <Sword size={18} className="text-accent" />;
  return <Trophy size={18} className="text-accent" />;
}

export default function Dashboard() {
  const { programs, convert, unitLabel } = useApp();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center py-4">
        <p className="text-[10px] uppercase tracking-[0.2em] text-accent font-semibold mb-3">Wisdom</p>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[10px] uppercase tracking-widest text-accent font-semibold">Today's Quest</p>
              </div>
              <h3 className="text-lg font-bold text-text truncate">{today.session_name}</h3>
              <p className="text-xs text-text-muted mt-0.5">
                Week {today.week} · {today.exercises?.length || 0} exercises
              </p>
            </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Kpi label="Sessions" value={week.sessions ?? 0} />
          <Kpi label="Volume" value={week.volume_kg != null ? Math.round(convert(week.volume_kg)).toLocaleString() : '0'} suffix={unitLabel} />
          <Kpi label="Streak" value={week.streak_days ?? week.streak ?? 0} icon={<Torch size={14} className="text-accent" />} />
        </div>
      </section>

      {/* Medals + Muscle ranks */}
      <div className="grid md:grid-cols-2 gap-4">
        <Card title="Medal showcase" action={<Link to="/medals" className="text-xs text-accent flex items-center gap-1">All <ArrowRight size={12} /></Link>}>
          {medals.length ? (
            ...
            <p className="text-sm text-text-muted">No medals held yet.</p>
          )}
        </Card>

        <Card title="Muscle ranks" action={<Link to="/profile" className="text-xs text-accent flex items-center gap-1">Profile <ArrowRight size={12} /></Link>}>
          {topRanks.length ? (
            ...
            <p className="text-sm text-text-muted">Train more to earn ranks.</p>
          )}
        </Card>
      </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-text-muted uppercase tracking-wider">Recent PRs</h3>
            </div>
